...
def GANTrainStep(model, batch, optimizers, criterion, cuda):
  ...
  zVals = Variable(torch.Tensor(batch.size(0), nz).normal_(0,1)) # Normalize here in a different train step *****
  if cuda:
...
# GAN inputs normalized to sphere
def NormGANTrainStep(model, batch, optimizers, criterion, cuda):
  netG, netD = model
  optimG, optimD = optimizers
  criterionG, criterionD = criterion
  ...
  onesLabel = Variable(torch.ones(batch.size(0)))
  zerosLabel = Variable(torch.zeros(batch.size(0)))
  zVals = torch.Tensor(batch.size(0), nz).normal_(0,1)
  zVals = zVals / torch.sqrt((zVals*zVals).sum(1, keepdim=True)) #Normalize to sphere!
  zVals = Variable(zVals) # Normalize here in a different train step *****
  if cuda:
    onesLabel = onesLabel.cuda()
    zerosLabel = zerosLabel.cuda()
    batch = batch.cuda()
...
def getTrainFunc(trainfunc, validation = False):
  if trainfunc == 'gan':
    return GANTrainStep
  elif trainfunc == 'normgan':
    return NormGANTrainStep
...
# python train.py --model dcgan --dataset mnist --netG /foo/bar --netD /foo/bar --criterion gan --trainFunc normgan
