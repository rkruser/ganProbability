...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train', embedding_type='cnn-rnn',
                 imsize=64, transform=None, target_transform=None, randomIndex = True):

        self.randomIndex = randomIndex #Added by Ryen
        self.transform = transform
        self.target_transform = target_transform
        self.imsize = imsize
        self.data = []
        self.data_dir = data_dir
        # The following path check is stupid -Ryen
...
#            caption_name = '%s/text/%s.txt' % (self.data_dir, key)
            caption_name = os.path.join(self.data_dir,os.path.join('text','%s.txt'%(key)))
...
#            data_dir = '%s/CUB_200_2011' % self.data_dir
   ...
   #     img_name = '%s/images/%s.jpg' % (data_dir, key)
        img_name = os.path.join(data_dir, os.path.join('images','%s.jpg'%key))
        img = self.get_img(img_name, bbox)


        # The following if statement is from Ryen
        embedding_ix = 0
        if self.randomIndex:
