...
def compute_loss():
    ...
    # generator loss
    gloss = torch.mean((torch.mean(data_preds, 0) - torch.mean(sample_preds, 0))**2)
...
# do checkpointing
torch.save(netG.state_dict(), '%s/netG_ALI_features_%s.pth' % (opt.outf, opt.dataset))
torch.save(netD.state_dict(), '%s/netD_ALI_features_%s.pth' % (opt.outf, opt.dataset))
torch.save(netE.state_dict(), '%s/netE_ALI_features_%s.pth' % (opt.outf, opt.dataset))
...
sio.savemat(os.path.join(opt.outf, 'ALI_features_%s.mat' % (opt.dataset)),
