...
from models import _lenet, _netF, generate_data, mog_netP, generate_adversarial_data, _lenet_ad, Q_net, P_net
...
parser.add_argument('--netF', default='', help="path to netF (to load model)")
...
if opt.dataset == 'mog':
    ...
    netE = Q_net(784, 1000, 8)
    netG = P_net(784, 1000, 8)
    netP = _netF(ngpu)
    netF = _lenet_ad(ngpu)
...
netE.load_state_dict(torch.load(os.path.join(opt.outf, 'AE_netQ_epoch_999.pth')))
netG.load_state_dict(torch.load(os.path.join(opt.outf, 'AE_netP_epoch_999.pth')))

filename = os.path.join(opt.outf, opt.netF)
if os.path.isfile(filename):
    print("==> loading params from checkpoint '{}'".format(filename))
    netF.load_state_dict(torch.load(filename))
else:
    print("==> no checkpoint found at '{}'".format(filename))
    raise
...
for i, (data,labels) in enumerate(dataloader, 0):
    ...
    datav = datav.view(datav.size(0),-1)
    # output = netE(datav)
    # output = netG(output)
    # output = (output - 0.5) / 0.5

    _,output = netF(datav)
    output = netP(output, 4.0)
...
sio.savemat(os.path.join(opt.outf, 'testoutput_%d.mat' % opt.classindex),
...
if opt.dataset != 'mog':
    ...
    vutils.save_image(torch.from_numpy(images[indices[:-101:-1]]), os.path.join(opt.outf, 'best_'+opt.dataset+'_image_%d.png' % opt.classindex),
                      nrow=10, normalize=True)
    vutils.save_image(torch.from_numpy(images[indices[:100]]), os.path.join(opt.outf, 'worst_'+opt.dataset+'_image_%d.png' % opt.classindex),
                      nrow=10, normalize=True)