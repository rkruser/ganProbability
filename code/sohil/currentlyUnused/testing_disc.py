...
from models import _netD, generate_data, mog_netD, _lenet, generate_adversarial_data, _lenet_ad
...
if opt.dataset == 'cifar10':
    ...
    dataset = generate_data(num_mode=8,except_num=1)
...
for i, (data,labels) in enumerate(dataloader, 0):
    ...
    prob[i * opt.batchSize:(i + 1) * opt.batchSize] = p.cpu().numpy().reshape(data.size(0))
    label[i * opt.batchSize:(i + 1) * opt.batchSize] = labels.numpy().reshape(data.size(0))
    pred[i * opt.batchSize:(i + 1) * opt.batchSize] = predicted.cpu().numpy().reshape(data.size(0))
...
sio.savemat(os.path.join(opt.outf, 'testoutput_classifier'),
...
vutils.save_image(torch.from_numpy(images[indices[:-101:-1]]), os.path.join(opt.outf, 'best_'+opt.dataset+'_image_classifier.png'),
                  nrow=10, normalize=True)
vutils.save_image(torch.from_numpy(images[indices[:100]]), os.path.join(opt.outf, 'worst_'+opt.dataset+'_image_classifier.png'),
                  nrow=10, normalize=True)