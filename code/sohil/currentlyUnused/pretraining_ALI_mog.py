...
def compute_loss():
    ...
    data_preds = netD(torch.cat([x, z_hat], 1))
    sample_preds = netD(torch.cat([x_hat, z], 1))

    # generator loss
    gloss = torch.mean(F.softplus(data_preds) + F.softplus(-sample_preds))

    data_preds = netD(torch.cat([x, z_hat.detach()], 1))
    sample_preds = netD(torch.cat([x_hat.detach(), z], 1))
...
# do checkpointing
torch.save(netG.state_dict(), '%s/netG_ALI_mog.pth' % (opt.outf))
torch.save(netD.state_dict(), '%s/netD_ALI_mog.pth' % (opt.outf))
torch.save(netE.state_dict(), '%s/netE_ALI_mog.pth' % (opt.outf))
