...
parser.add_argument('--niter', type=int, default=75000, help='number of epochs to train for')
...
def compute_loss():
    ...
    _, recon_preds = netD(torch.cat([x_recon.detach(), z_hat.detach()], 1))
...
for epoch in range(opt.niter):
    real_cpu = generate_data_single_batch(num_mode=8, except_num=2, radius=2, batch_size=opt.batchSize)
...
# do checkpointing
torch.save(netG.state_dict(), '%s/netG_AugALI_mog.pth' % (opt.outf))
torch.save(netD.state_dict(), '%s/netD_AugALI_mog.pth' % (opt.outf))
torch.save(netE.state_dict(), '%s/netE_AugALI_mog.pth' % (opt.outf))
...
for i, (real_cpu,labels) in enumerate(dataloader, 0):
    ...
    x.data.resize_as_(real_cpu).copy_(real_cpu)
    z.data.resize_(batch_size, nz).normal_(0,1)
    ## TODO: Please change to noisy input  if fill of 0 do not work well
    # noise.data.resize_(batch_size, nz).normal_(0,1)
    noise.data.resize_(batch_size, nz).fill_(0)
...
sio.savemat(os.path.join(opt.outf, 'AugALI_mog.mat'),
