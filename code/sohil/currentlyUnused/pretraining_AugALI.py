...
class _netD(nn.Module):
    ...
    def forward(self, image, latent):
        ...
        if isinstance(image.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            inter = nn.parallel.data_parallel(self.joint1, torch.cat([outputX.view(outputX.size(0), -1), outputZ],
                                                                     dim=1), range(self.ngpu))
            output = nn.parallel.data_parallel(self.joint2, inter, range(self.ngpu))
        else:
            outputX = self.discX(image)
            outputZ = self.discZ(latent)
            inter = self.joint1(torch.cat([outputX.view(outputX.size(0), -1), outputZ], dim=1))
            output = self.joint2(inter)

        return inter, output.view(-1, 1).squeeze(1)
...
def compute_loss():
    ...
    x_hat, z_hat, x_recon = sampling()

    data_preds, _ = netD(x, z_hat)
    sample_preds, _ = netD(x_hat, z)

    # generator loss
    gloss = torch.mean(torch.abs(torch.mean(data_preds, 0) - torch.mean(sample_preds, 0)))

    _, data_preds = netD(x, z_hat.detach())
    _, sample_preds = netD(x_hat.detach(), z)
    _, recon_preds = netD(x_recon.detach(), z_hat.detach())

    # discriminator loss
    dloss = torch.mean(F.softplus(-data_preds) + 0.5 * F.softplus(sample_preds) + 0.5 * F.softplus(recon_preds))
...
# do checkpointing
torch.save(netG.state_dict(), '%s/netG_AugALI_%s.pth' % (opt.outf, opt.dataset))
torch.save(netD.state_dict(), '%s/netD_AugALI_%s.pth' % (opt.outf, opt.dataset))
torch.save(netE.state_dict(), '%s/netE_AugALI_%s.pth' % (opt.outf, opt.dataset))
...
sio.savemat(os.path.join(opt.outf, 'AugALI_%s.mat' % (opt.dataset)),
