...
from code.sohil.models import _netP, prob_data, weights_init, mog_netP
from code.sohil.utils import AverageMeter
...
class RegressorTrainingRyen(Operator):
  ...
  def run(self):
    trainloader, testloader = self.dataLoader.getProbData()
    problem = self.modelLoader.getRegressorProblem()
    netP = problem['netP']
