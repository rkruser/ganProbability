...
from ModelTemplate import ModelTemplate, AverageMeter
from nnModels import weights_init, NetG28, NetD28
...
class DCGAN28(ModelTemplate):
  def __init__(self, config, args):
    super(DCGAN28,self).__init__(config, args)
    self.opt = {
      'nz': 100,
      'ngf':64,
      'ndf':64,
    ...
    self.netG = NetG28(nz=self.nz, ngf=self.ngf, nc=self.nc, ngpu=self.ngpu)
    self.netD = NetD28(nz=self.nz, ngf=self.ngf, nc=self.nc, ngpu=self.ngpu)
  ...
  def probSample(self, nSamples, deepFeatures=None, method='numerical'):
    codes = torch.FloatTensor(nSamples,self.nz).normal_(0,1)
    results = self.getProbs(codes, method=method)
    results['codes'] = codes
  ...
  def getProbs(self, codes, method='numerical', epsilon=1e-5):
    assert(len(codes.size()) == 4)

    if method=='numerical':
      noise = torch.FloatTensor(2*self.nz+1,nz,1,1)
    else:
    ...
    images = np.empty([nsamples, self.nc, 28, 28])
  ...
  def saveCheckpoint(self, checkpointNum):
